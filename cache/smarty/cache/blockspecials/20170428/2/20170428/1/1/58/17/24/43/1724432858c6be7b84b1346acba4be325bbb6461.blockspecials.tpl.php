<?php /*%%SmartyHeaderCode:384559038989738546-33087391%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '1724432858c6be7b84b1346acba4be325bbb6461' => 
    array (
      0 => 'C:\\WebServer\\Apache2.2\\htdocs\\promaxilaser\\themes\\default-bootstrap\\modules\\blockspecials\\blockspecials.tpl',
      1 => 1489520958,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '384559038989738546-33087391',
  'version' => 'Smarty-3.1.19',
  'unifunc' => 'content_59038b974fabc1_64126681',
  'has_nocache_code' => false,
  'cache_lifetime' => 31536000,
),true); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_59038b974fabc1_64126681')) {function content_59038b974fabc1_64126681($_smarty_tpl) {?>
<!-- MODULE Block specials -->
<div id="special_block_right" class="block">
	<p class="title_block">
        <a href="http://localhost/promaxilaser/index.php?controller=prices-drop" title="Promoções">
            Promoções
        </a>
    </p>
	<div class="block_content products-block">
    		<ul>
        	<li class="clearfix">
            	<a class="products-block-image" href="http://localhost/promaxilaser/index.php?id_product=7&amp;controller=product">
                    <img 
                    class="replace-2x img-responsive" 
                    src="http://localhost/promaxilaser/img/p/2/0/20-small_default.jpg" 
                    alt="" 
                    title="Vestido de chiffon estampado" />
                </a>
                <div class="product-content">
                	<h5>
                        <a class="product-name" href="http://localhost/promaxilaser/index.php?id_product=7&amp;controller=product" title="Vestido de chiffon estampado">
                            Vestido de chiffon estampado
                        </a>
                    </h5>
                                        	<p class="product-description">
                            Vestido chiffon de alça estampado na...
                        </p>
                                        <div class="price-box">
                    	                        	<span class="price special-price">
                                                                    R$ 20,50                            </span>
                                                                                                                                 <span class="price-percent-reduction">-20%</span>
                                                                                         <span class="old-price">
                                                                    R$ 25,63                            </span>
                            
                                            </div>
                </div>
            </li>
		</ul>
		<div>
			<a 
            class="btn btn-default button button-small" 
            href="http://localhost/promaxilaser/index.php?controller=prices-drop" 
            title="Todas as promoções">
                <span>Todas as promoções<i class="icon-chevron-right right"></i></span>
            </a>
		</div>
    	</div>
</div>
<!-- /MODULE Block specials -->
<?php }} ?>
